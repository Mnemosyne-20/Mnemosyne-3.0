...
namespace ArchiveApi
{
    public class TimeMap
    {
        ...
        public TimeMap(WebLink web) : this(web.Link)
        {
            if (web.Parameters["rel"].Contains("self"))
            {
                Until = DateTime.Parse(string.Join(" ", web.Parameters["until"]));
                From = DateTime.Parse(string.Join(" ", web.Parameters["from"]));
            }
            else
            {
        ...
        public static async Task<Mementos> GetMementosAsync(IArchiveService service, Uri url)
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                mementos = new Mementos(ParseWebLinks.Parse(await response.Content.ReadAsStringAsync()));
            }
            return mementos;
        }
